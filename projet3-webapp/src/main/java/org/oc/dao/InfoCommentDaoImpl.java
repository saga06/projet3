...
public class InfoCommentDaoImpl implements InfoCommentDao {
    ...
    InfoCommentDaoImpl(DaoFactory daoFactory) {
        this.daoFactory = daoFactory;
    }


    public void ajouter(InfoComment infoComment) {
        ...
        try {
            ...
            preparedStatement = connexion.prepareStatement("INSERT INTO comment( content, site_id, user_nickname) VALUES(?, ?, ?);");
            preparedStatement.setString(1, infoComment.getContent());
            preparedStatement.setInt(2, infoComment.getSite_id());
            preparedStatement.setString(3, infoComment.getUser_nickname());
    ...
    @Override
    public List<InfoComment> lister(int ID) {
        List<InfoComment> infoComments = new ArrayList<InfoComment>();
        Connection connexion = null;
        ...
        try {
            connexion = daoFactory.getConnection();
            ps = connexion.prepareStatement("SELECT content, site_id, user_nickname FROM comment WHERE site_id=?");
            ps.setInt(1, ID);
            ...
            while (resultat.next()) {
                ...
                String user_nickname = resultat.getString("user_nickname");
                ...
                infoComment.setContent(content);
                infoComment.setUser_nickname(user_nickname);
                infoComment.setSite_id(site_id);
